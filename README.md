...
int main()
{
    ...
    // 객체를 자동으로 풀에 반납
    // (혹은 unique_ptr의 reset()을 쓰지 않고도 스코프를 벗어나면서 자동으로 소멸도 가능)
    // unique_ptr의 RAII 특성 참고
    obj1.reset();

    return 0;
}
```
위 코드는 객체를 반납하는 코드입니다. unique_ptr가 스코프를 벗어나거나 명시적으로 ```reset()```이 호출될 때, 커스텀 ```deleter```가 호출되어 자동으로 오브젝트풀에 객체가 반납됩니다.
